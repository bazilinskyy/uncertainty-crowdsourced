...
class Analysis:
        ...
        if pretty_text:
            for variable in y:
                ...
                if isinstance(df.iloc[0][variable], str):
                    ...
                    df[variable] = df[variable].suc.replace('_', ' ')
                    # capitlise
                    df[variable] = df[variable].suc.capitalize()
    ...
    def scatter(self, df, x, y, color=None, symbol=None, size=None, text=None,
        ...
        if pretty_text:
            if isinstance(df.iloc[0][x], str):  # check if string
                # replace underscores with spaces
                df[x] = df[x].suc.replace('_', ' ')
                # capitlise
                df[x] = df[x].suc.capitalize()
            if isinstance(df.iloc[0][y], str):  # check if string
                # replace underscores with spaces
                df[y] = df[y].suc.replace('_', ' ')
                # capitlise
                df[y] = df[y].suc.capitalize()
            if color and isinstance(df.iloc[0][color], str):  # check if string
                # replace underscores with spaces
                df[color] = df[color].suc.replace('_', ' ')
                # capitlise
                df[color] = df[color].suc.capitalize()
            if size and isinstance(df.iloc[0][size], str):  # check if string
                # replace underscores with spaces
                df[size] = df[size].suc.replace('_', ' ')
                # capitlise
                df[size] = df[size].suc.capitalize()
            try:
                # check if string
                if text and isinstance(df.iloc[0][text], str):
                    # replace underscores with spaces
                    df[text] = df[text].suc.replace('_', ' ')
                    # capitlise
                    df[text] = df[text].suc.capitalize()
        ...
        if pretty_text:
            for x_col in x:
                if isinstance(df.iloc[0][x_col], str):  # check if string
                    # replace underscores with spaces
                    df[x_col] = df[x_col].suc.replace('_', ' ')
                    # capitlise
                    df[x_col] = df[x_col].suc.capitalize()
            if isinstance(df.iloc[0][y], str):  # check if string
                # replace underscores with spaces
                df[y] = df[y].suc.replace('_', ' ')
                # capitlise
    ...
    def hist_stim_duration_time(self, df, time_ranges, nbins=0,
        ...
        col_dur = df.columns[df.columns.to_series().suc.contains('-dur')]
